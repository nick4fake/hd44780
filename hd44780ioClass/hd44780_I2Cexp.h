...
// A bug in TinyWire2M is that requestFrom() returns incorrect status
// so its return status can't be used. Instead the code will check the return
// from Wire2.read() which will return -1 if there no data was transfered.
...
// if Wire2 library is used in constructor, it will hang.
int ioinit()
{
int status = 0;
...
static uint8_t AutoInst;
	...
	/*
	 * First, initialize the i2c (Wire2) library.
	 * This really shouldn't be here 
	 * because Wire2.begin() should only be called once, but
	...
	if(_expType == I2Cexp_MCP23008)
	{
		 ...
		 */
		Wire2.write(5);	// point to IOCON
		Wire2.write(0x20);// disable sequential mode (enables BYTE mode)
		 ...
		 */
		Wire2.beginTransmission(_addr);
		Wire2.write((uint8_t)0); // point to IODIR
		Wire2.write((uint8_t)0); // all pins output
		Wire2.endTransmission();
		 ...
		 */
		Wire2.beginTransmission(_addr);
		Wire2.write(9); // point to GPIO
		
	}
	Wire2.write((uint8_t)0);  // Set the entire output port to LOW
	if( (status = Wire2.endTransmission()) ) // assignment
...
int rval = hd44780::RV_EIO;
	...
	Wire2.beginTransmission(_addr);
	Wire2.write(gpioValue);		// d4-d7 are inputs, RS, r/w high, E LOW
	if(Wire2.endTransmission())
		goto returnStatus;
	...
	Wire2.beginTransmission(_addr);
	Wire2.write(gpioValue | _en); // Raises E 
	if(Wire2.endTransmission())
		goto returnStatus;

	// read the expander port to get the upper nibble of the byte
	Wire2.requestFrom((int)_addr, 1);
	iodata = Wire2.read();
	if(iodata < 0) // did we not receive a byte?
		goto returnStatus;

	Wire2.beginTransmission(_addr);
	Wire2.write(gpioValue); // lower E after reading nibble
	if(Wire2.endTransmission())
		goto returnStatus;
	...
	Wire2.beginTransmission(_addr);
	Wire2.write(gpioValue | _en); // Raise E to read next nibble
	if(Wire2.endTransmission())
		goto returnStatus;

	// read the expander port to get the lower nibble of the byte
	// We can't look at the return value from requestFrom() on the TineyWire2M
...
returnStatus:
	...
	Wire2.beginTransmission(_addr);
	Wire2.write(_blCurState);		// with E LOW
	if(Wire2.endTransmission())
		rval = hd44780::RV_EIO;
...
int iowrite(hd44780::iotype type, uint8_t value) 
{
	...
	if(Wire2.endTransmission()) // send buffered bytes to the expander
		return(hd44780::RV_EIO);
...
I2CexpType chiptype;
	 ...
	 */
	Wire2.beginTransmission(address);
	Wire2.write((uint8_t) 0);	// try to point to MCP23008 IODR
	Wire2.write((uint8_t) 0xff);	// try to write to MCP23008 IODR
	Wire2.endTransmission();
...
uint8_t blLevel;
	...
	Wire2.beginTransmission(_addr);
	Wire2.write((uint8_t)0); // point to IODIR
	Wire2.write(0xff); // all pins inputs
	Wire2.endTransmission();

	Wire2.beginTransmission(_addr);
	Wire2.write(6); // point to GPPU
...
uint8_t gpioValue =  _blCurState;
	...
	Wire2.write(gpioValue |_en);	// with E HIGH
	Wire2.write(gpioValue);		// with E LOW
}
